...
export class Scope {
    parent: Scope
    [key: string]: any //address

    valueSet(value: string, address) {
        this[value] = address;
    }
    valueGet(value: string, scope: Scope) {
        if (scope[value]) {
            return this.getValueByAddress(scope[value]);
        } else {
            if (scope.parent) {
                return this.valueGet(value, this.parent);
            } else {
                console.error(`${value} is not define`);
            }
        }
    }
