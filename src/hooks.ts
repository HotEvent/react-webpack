...
function useBehaviorSubject<T>(bhs: BehaviorSubject<T>) {
    const [state, setState] = useState(bhs.value);
    useEffect(() => {
        let sub = bhs.pipe(filter(value => value !== state)).subscribe(value => setState(value));
